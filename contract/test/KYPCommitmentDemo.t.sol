...
contract KYPCommitmentDemoTest is Test {
    KYPCommitmentDemo kyp;
    ...
    function setUp() public {
        kyp = new KYPCommitmentDemo(verifier);
    ...
    function test_StakeCreatesActiveCommitment() public {
        ...
        KYPCommitmentDemo.Commitment memory c = kyp.getCommitment(id);
        assertEq(c.user, user);
        assertEq(c.protocolAddress, protocol);
        assertEq(c.amount, 1 ether);
        assertEq(uint256(c.status), uint256(KYPCommitmentDemo.Status.Active));
    ...
    function test_VerifyReturnsStakeBeforeDeadline() public {
        ...
        KYPCommitmentDemo.Commitment memory c = kyp.getCommitment(id);
        assertEq(uint256(c.status), uint256(KYPCommitmentDemo.Status.Verified));
    ...
    function test_SlashForfeitsStakeAfterDeadline() public {
        ...
        KYPCommitmentDemo.Commitment memory c = kyp.getCommitment(id);
        assertEq(uint256(c.status), uint256(KYPCommitmentDemo.Status.Slashed));
        assertEq(kyp.activeStakedTotal(), 0);
