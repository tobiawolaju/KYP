...
const { query, update } = require("./services/db");
...
async function runCheck(commitment) {
  ...
  console.log(`[WORKER] Commitment ${commitment.id} -> missed_count: ${newMissedCount}/${MAX_MISSED_CHECKS}`);
  return result;
}
...
async function tick() {
  ...
  for (const commitment of allCommitments) {
    try {
      ...
      if (now >= deadline) {
        console.log(`[WORKER] Commitment ${commitment.id} past deadline, skipping`);
        continue;
      }
      ...
      console.error(`[WORKER] Error checking commitment ${commitment.id}:`, err.message);
    }
  }
